...
# def my_error_handler(client, update, error):
#     print("An error occurred:", error)
#     client.send_message(-1001426113453, f"An error occurred:\n`{error}`")
#
#
# app.add_handler(pyrogram.handlers.MessageHandler(my_error_handler))
...
async def start_bot():
    async with app:
            ...
            if i.endswith(".py"):
                ...
                try:
                    ...
                    try:
                        ...
                        filename = "error_" + ''.join(
                            random.choices(string.ascii_uppercase + string.digits, k=8)) + ".txt"
                        with open(filename, "w", encoding='utf-8') as f:
                            f.write(str(traceback_msg))
        ...
        @app.on_message(filters.all)
        @error_handler
        async def check_for_it(client, message):
            await _1check_for_it(client, message)
...
# Run the bot using the async wrapper function
