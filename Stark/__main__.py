...
async def initial():
        ...
        if i.endswith(".py"):
            ...
            try:
                ...
                try:
                    print(app)
                    print(type(app))
                    mg = await app.send_message(-1001426113453, traceback_msg)
                    lin = mg.link
                except Exception:
                    filename = "error_" + ''.join(random.choices(string.ascii_uppercase + string.digits, k=8)) + ".txt"
                    with open(filename, "w", encoding='utf-8') as f:
                        f.write(str(traceback_msg))
                    mg = await app.send_document(chat_id=-1001426113453, document=filename)
                    lin = mg.link
                    os.remove(filename)
    ...
    try:
        mg = await app.send_message(-1001426113453, text=mgt)
        url = mg.link
    except:
        ...
        mg = await app.send_document(chat_id=-1001426113453, document=filename, reply_markup=types.InlineKeyboardMarkup(
