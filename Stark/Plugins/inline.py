...
buttons = [
  ...
  [
    InlineKeyboardButton("Youtube (other chat)", switch_inline_query = "yt "),InlineKeyboardButton("Youtube (this chat)", switch_inline_query_current_chat = "yt ")
  ],
  [
    InlineKeyboardButton("Apps (othrr chat)", switch_inline_query = "app "),InlineKeyboardButton("Apps (this chat)", switch_inline_query_current_chat = "app ")
  ]
]
...
thumb = f"https://img.youtube.com/vi/ {
  yt_id
}/hqdefault.jpg"
capt = f"""
➥ **Title:** ` {
...
app_rating = (
f" {
  result['score']}/5 ⭐️" if result["score"] else "3.5/5 ⭐️"
)
...
app_details = f"[📲]( {
  ...
  app_desc
}`\n**𝖥𝗎𝗅𝗅 𝖣𝖾𝗍𝖺𝗂𝗅𝗌:** [𝖵𝗂𝖾𝗐 𝖮𝗇 𝖯𝗅𝖺𝗒 𝖲𝗍𝗈𝗋𝖾]( {
  app_link
})"
result.append(
  InlineQueryResultArticle(
